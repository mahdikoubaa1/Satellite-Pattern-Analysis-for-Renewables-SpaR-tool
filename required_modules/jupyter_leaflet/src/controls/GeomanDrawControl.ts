...
export class LeafletGeomanDrawControlView extends LeafletControlView {
  feature_group_r: GeoJSON;
  feature_group_b: GeoJSON;
  ...
  create_obj() {
    this.feature_group_r = L.geoJson([], {
    ...
    this.feature_group_b=L.geoJson([], {
      style: function (feature) {
        if (feature?.properties != undefined) {
          return feature.properties.style;
        } else {
    ...
    this.data_to_layers();
    this.map_view.obj.addLayer(this.feature_group_r);
    this.map_view.obj.addLayer(this.feature_group_b);
    var vis=this.model.get('visible')
    ...
    this.listenTo(this.model, 'change:polygon', () => {
      drawPolygon = this.model.get('polygon');
      if ('shapeOptions' in drawPolygon) {
        drawPolygon.pathOptions = drawPolygon.shapeOptions;
        delete drawPolygon.shapeOptions;
      }
      ...
      if ('d' in drawPolygon.pathOptions && drawPolygon.pathOptions.d){
        const currentShape = this.map_view.obj.pm.Draw.getActiveShape();
        if (currentShape) {
          this.map_view.obj.pm.disableDraw(currentShape);
        }
        this.map_view.obj.pm.Toolbar.set('drawPolygon',true);
      }else{
        this.map_view.obj.pm.Toolbar.setButtonDisabled('drawPolygon',false);
      }
      
    
    });
    this.listenTo(this.model, 'change:visible', () => {
      if (vis.s !=this.model.get('visible').s){
        vis.s=this.model.get('visible').s
        if (vis.s){
          this.map_view.obj.addLayer(this.feature_group_r);
            
        }
        else{
          this.map_view.obj.removeLayer(this.feature_group_r);
        }
      }
      if (vis.ns !=this.model.get('visible').ns){
        vis.ns=this.model.get('visible').ns
        if (vis.ns){
          this.map_view.obj.addLayer(this.feature_group_b);

        }
        else{
          this.map_view.obj.removeLayer(this.feature_group_b);

        }
      }
      
    });
    var newpolygon = this.model.get('newpolygon');

    this.listenTo(this.model, 'change:newpolygon', () => {
      newpolygon = this.model.get('newpolygon');
      if ('properties' in newpolygon && 'label' in newpolygon.properties)
      {
      if (newpolygon.properties.label==0) this.feature_group_b.addData (newpolygon);
      else {
        this.feature_group_r.addData (newpolygon);
    ...
    this.controlOptions = {
      ...
      drawRectangle: !(drawRectangle == false || ('visibility' in drawRectangle && drawRectangle.visibility==false)),
      drawPolygon: !(drawPolygon == false || ('visibility' in drawRectangle && drawRectangle.visibility==false)),
    ...
    this.map_view.obj.on(
      ...
      (e: { shape: L.PM.SUPPORTED_SHAPES; layer: LayerShapes }) => {
        var layer = e.layer;
        if(!(layer   instanceof L.Rectangle) && layer instanceof L.Polygon && ((layer.getBounds().getNorthEast().lat>drawPolygon.pathOptions.bounds[1][0])||(layer.getBounds().getNorthEast().lng>drawPolygon.pathOptions.bounds[1][1])||(layer.getBounds().getSouthWest().lat<drawPolygon.pathOptions.bounds[0][0])||(layer.getBounds().getSouthWest().lng<drawPolygon.pathOptions.bounds[0][1])) ) {
          var d =layer.getCenter();

          this.map_view.obj.removeLayer(layer);
          L.popup()
              .setLatLng(d)
              .setContent('Not in raster')
              .openOn(this.map_view.obj);
        ...
        if (e.shape === 'Text') {
          ...
          layer.on('pm:textblur', (e) => {
            ...
            this.feature_group_r.addLayer(layer);
            this.layers_to_data();
            this.model.save_changes();
          });
          return;
        }
        if (layer instanceof L.Polygon && layer.options.color=='red'){
          this.feature_group_r.addLayer(layer);
        }
        else this.feature_group_b.addLayer(layer);
    ...
    this.map_view.obj.on(
      ...
      (e: { shape: L.PM.SUPPORTED_SHAPES; layer: LayerShapes }) => {
        ...
        if (eventLayer instanceof L.Polygon && eventLayer.options.color=='red'){
          this.feature_group_r.removeLayer(eventLayer);
        }
        else if (eventLayer instanceof L.Polygon && eventLayer.options.color=='blue') this.feature_group_b.removeLayer(eventLayer);
    ...
    this.map_view.obj.on(
      ...
      (e: {
        ...
        var geo_json = [];
        this.feature_group_r.eachLayer((layer) => {
          if (layer._leaflet_id == e.originalLayer._leaflet_id) {
            this.feature_group_r.removeLayer(layer);
          } else {
            geo_json.push(this.layer_to_json(layer));
          }
        });
        this.feature_group_b.eachLayer((layer) => {
          if (layer._leaflet_id == e.originalLayer._leaflet_id) {
            this.feature_group_b.removeLayer(layer);
          } else {
            geo_json.push(this.layer_to_json(layer));
          }
        });
        if (eventLayer instanceof L.Polygon && eventLayer.options.color=='red') this.feature_group_r.addLayer(eventLayer);
        else if (eventLayer instanceof L.Polygon && eventLayer.options.color=='blue') this.feature_group_b.addLayer(eventLayer);
  ...
  data_to_layers() {
    ...
    this.feature_group_r.clearLayers();
    this.feature_group_b.clearLayers();
    this.feature_group_r.addData(data);
    this.feature_group_b.addData(data);

    // We add event listeners here, since these need to be added on a
    // per-layer basis.
    this.feature_group_r.eachLayer((layer) => {
      if(layer instanceof L.Polygon && layer.options.color=='blue'){
        this.feature_group_r.removeLayer(layer);
        return;
      }
