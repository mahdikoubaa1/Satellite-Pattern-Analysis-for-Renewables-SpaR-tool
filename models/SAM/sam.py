...
from models.SAM import register
...
class FocalLoss(nn.Module):
    ...
    def forward(self, pred, gt):
        ...
        gt = gt.float()  # Ensure ground truth is float

        # Compute the focal loss components
        pos_loss = -self.alpha * (1 - pred_prob) ** self.gamma * gt * torch.log(pred_prob + 1e-8)
        neg_loss = -(1 - self.alpha) * pred_prob ** self.gamma * (1 - gt) * torch.log(1 - pred_prob + 1e-8)
...
class SAM(nn.Module):
    def __init__(self, inp_size=None, encoder_mode=None, loss=None):
        ...
        # loss = 'focal' #toRemove
        self.device = torch.device("cpu") #importent to change if cpu/cuda
